...
m_file = open('nodePath_Test_case_1.txt', "w+")                       # open text file
...
solved_mat, path = path_finder([[1, 2, 3, 4],[ 5, 6,0, 8], [9, 10, 7, 12] , [13, 14, 11, 15]])

print("solved matrix",solved_mat)                      # Displaying solved matrix
print("path taken is:")
for o in range(len(path)):                             # Displaying the path from initial to final position
    print(" ")
    for p in range(4):
        print(path[o][p])
end_time = time.time()
