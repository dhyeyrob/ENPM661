...
plotting_ROS_too = True  # true if want to use ROS
Fast_RRT = False #true for fast rrt, false for basic rrt <------------TESTER SET FAST/BASIC RRT FLAG HERE



if plotting_ROS_too == True:  # give ROS time to load up
    print("\nCode will wait 1 minute to allow ROS/Gazebo to startup\n")    
    time.sleep(60)
    import rospy
...
xscale = 10 * scaling
VisitedDict = {}
total_clearance = 0.12 * scaling  #clearance of 12.5 m in the scaled space

# Get req'd inputs from tester
Theta_Start = 0  # <--------------------------TESTER PUT INITIAL HEADING ANGLE IN DEGREES HERE
TestCaseCOOR = [1,5]  # <---------------------TESTER PUT INITIAL X,Y COORDINATE PT HERE
FinalStateCOOR = [5,9]  # <-------------------TESTER PUT GOAL X,Y COORDINATE PT HERE
...
fps = 5
if Fast_RRT == True:
    outputVideo = cv2.VideoWriter("Sweeping_Fast_RRT.mp4", fourcc, fps, (xscale, yscale))  # output resolution must match input frame (60% resized from 1980x1020)
if Fast_RRT == False:
    outputVideo = cv2.VideoWriter("Sweeping_Basic_RRT.mp4", fourcc, fps, (xscale, yscale))  # output resolution must match input frame (60% resized from 1980x1020)
print("\n...Creating Video...\n")
...
if Fast_RRT == True: 
    ...
    #Optimal path is going North-East (dn->right)
    ...
    #Optimal path is going South-East (up->right)
    if x_diff >= 0 and y_diff <= 0:
        x_max = xscale
        x_min = initial_state[1] - 1
    ...
    #Optimal path is going North-West (dn->left) 
    if x_diff <= 0 and y_diff >= 0:
        x_min = 0
    ...
    #Optimal path is going South-west (up->left) 
    if x_diff <= 0 and y_diff <= 0:
        x_min = 0
        x_max = initial_state[1] + 1
        y_max = initial_state[0] + 1
...
def End(OurMap):
    ...
    print("\nRun time: ", runtime)
    if Fast_RRT == True: 
        print("\nDone. Can see results in Sweeping_Fast_RRT.mp4!")
    else: print("\nDone. Can see results in Sweeping_Basic_RRT.mp4!")
    sys.exit()
...
def Talker(OurMap):
    ...
    for o in range(0, len(OurMap)-1):
        ...
        msg.linear.x = 0  #added for stability between moves
        pub.publish(msg)
        pair = VisitedDict[OurMap[o]][1], VisitedDict[OurMap[o]][2] / scaling
        pair = tuple(pair)
...
def Get_Thetas(OurMap):
    ...
    for u in range(0, len(OurMap) - 1):
        ...
        slope = edge[0]
        theta = -1* math.atan(slope)
        
        #for vert lines
        if OurMap[u][1] == OurMap[u + 1][1]:
            if OurMap[u][0] < OurMap[u + 1][0]: theta = 270
            else: theta = 90
        
        #for horiz lines
        if OurMap[u][0] == OurMap[u + 1][0]:
            if OurMap[u][1] < OurMap[u + 1][1]: theta = 0
            else: theta = 180
    ...
    counter=0
    for u in range(0, len(OurMap) - 1):
        thetadot = thetadot_list[counter]
        counter+=1
...
def get_edge(alpha_i, q_nearest):
    ...
    #in case the edge is completely vertical 
    if np.abs(alpha_i[1] - q_nearest[1]) < 10:
        if alpha_i[0] > q_nearest[0]:
            high = alpha_i[0]
            low = q_nearest[0]+1
        else: 
            high = q_nearest[0]
        ...
        for e in range(low,high):
            edge_open_list.append((e,alpha_i[1]))
            
    #otherwise treat edge as a normal 1st degree polynomial 
    else: 
...
def Agg_Ext_Strat(edge_open_list, alpha_i, q_f):
    ...
    if len(clean_edge_open_list) >= 6: #if some reasonably large branch can be added
        ...
        if alpha_i_new in Cspace: Cspace.remove(alpha_i_new)
        # draw new edge and vertex
        draw_it(alpha_i, alpha_i_new, mask)


def x_traj(start, q_f, RRT, edges):
    point2= [start[0], q_f[1]]
    alpha_x = tuple(point2)  # y from alpha_y, x from q_f
    edge_open_list = get_edge(alpha_x, start)
    edge_open_list = order_open_list(edge_open_list, start)
    for child in edge_open_list:
        VisitedDict[child] = []
        VisitedDict[child].append(start)
    alpha_x, clean_edge_open_list = collision_free_edges(edge_open_list, alpha_x, start)
    return alpha_x, clean_edge_open_list 

    
def y_traj(start, q_f, RRT, edges):
    point1 = [q_f[0],start[1]]  
    alpha_y = tuple(point1) # y from q_f, x from q_0
    edge_open_list=[]
    if dn_left == True or dn_right == True:
        for e in range(start[0]+1,q_f[0]):
            edge_open_list.append((e,start[1]))
    elif up_left == True or up_right == True:
        for e in range(q_f[0]+1,start[0]):
            edge_open_list.append((e,start[1]))
    ...
    for child in edge_open_list:
        VisitedDict[child] = []
        VisitedDict[child].append(start)
    alpha_y, clean_edge_open_list = collision_free_edges(edge_open_list, alpha_y, start)
    return alpha_y, clean_edge_open_list


def trajectory_template(q_0, q_f, RRT, edges):
    
    #len 1
    start = q_0
    alpha_y1, clean_edge_open_list1  = y_traj(start, q_f, RRT, edges)
    start = alpha_y1
    alpha_x1, clean_edge_open_list2 = x_traj(start, q_f, RRT, edges)
    len1 = len(clean_edge_open_list1) + len(clean_edge_open_list2)
    
    #len2
    start = q_0
    alpha_x2, clean_edge_open_list3 = x_traj(start, q_f, RRT, edges)
    start = alpha_x2
    alpha_y2, clean_edge_open_list4  = y_traj(start, q_f, RRT, edges)
    len2 = len(clean_edge_open_list3) + len(clean_edge_open_list4)
      
    #choose the template that makes the largest 2 branches
    if len1 > len2: 
        if alpha_y1 not in RRT: RRT.append(alpha_y1)
        if alpha_x1 not in RRT: RRT.append(alpha_x1)
        if alpha_y1 in Cspace: Cspace.remove(alpha_y1)
        if alpha_x1 in Cspace: Cspace.remove(alpha_x1)
        for clean in clean_edge_open_list1: edges.append(clean)
        VisitedDict[alpha_y1] = []
        VisitedDict[alpha_y1].append(q_0)    
        for clean in clean_edge_open_list2: edges.append(clean)  # can now add the open edge list to the master edge list
        VisitedDict[alpha_x1] = []
        VisitedDict[alpha_x1].append(alpha_y1)  # make nearest node in tree the parent of alpha_i
        draw_it(q_0, alpha_y1, mask)
        draw_it(alpha_y1, alpha_x1, mask)
        parent = alpha_y1
        tip = alpha_x1
        
       
    else: 
        if alpha_y2 not in RRT: RRT.append(alpha_y2)
        if alpha_x2 not in RRT: RRT.append(alpha_x2)
        if alpha_y2 in Cspace: Cspace.remove(alpha_y2)
        if alpha_x2 in Cspace: Cspace.remove(alpha_x2)
        for clean in clean_edge_open_list3: edges.append(clean)
        VisitedDict[alpha_x2] = []
        VisitedDict[alpha_x2].append(q_0)    
        for clean in clean_edge_open_list4: edges.append(clean)  # can now add the open edge list to the master edge list
        VisitedDict[alpha_y2] = []
        VisitedDict[alpha_y2].append(alpha_x2)
        draw_it(q_0, alpha_x2, mask)
        draw_it(alpha_x2, alpha_y2, mask)
        parent = alpha_x2
        tip = alpha_y2
    
    #have to check in case tip = q_f --connected fully to goal 
    if tip == q_f:    
        VisitedDict[q_f] = []
        VisitedDict[q_f].append(parent)  # make nearest node in tree the parent of alpha_i
        search_time_stop = datetime.datetime.now()
        print("\nTime to find Goal: ", search_time_stop - starttime)
        roadmap(q_f)  #done exploring the configuration space  
        
    #otherwise try to connect to goal from tip 
    else: 
        not_straight, edge_open_list = clean_path_check(tip, q_f) #get a path from alpha to goal
        if not_straight == 0: #full path exists
            draw_it(tip, q_f, mask)
            VisitedDict[q_f] = []
            VisitedDict[q_f].append(tip)  # make nearest node in tree the parent of alpha_i
            RRT.append(q_f)
            search_time_stop = datetime.datetime.now()
            print("\nTime to find Goal: ", search_time_stop - starttime)
            roadmap(q_f)  #done exploring the configuration space  
        else: Agg_Ext_Strat(edge_open_list, tip, q_f)    
...
#rule-based trajectory template
if Fast_RRT == True:
    trajectory_template(q_0, q_f, RRT, edges)
...
while True:
    ...
    if clean_edge_open_list != []:
        ...
        if Fast_RRT == True: 
            Agg_Ext_Strat(edge_open_list, alpha_i, q_f)
