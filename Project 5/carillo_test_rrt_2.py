...
def in_circle(Node):
    ...
    #     return True
    if (x - (2 * scaling)) ** 2 + (y - (yscale - (8 * scaling))) ** 2 <= (r + total_clearance) ** 2:
        return True
    else:
        return False
...
def Get_Thetas(OurMap):
    for u in range(0, len(OurMap) - 1):
        edge = np.polyfit([OurMap[u][1], OurMap[u + 1][1]], [OurMap[u][0], OurMap[u + 1][0]],
                          1)  # construct edge from q_nearest to alpha_i
        slope = edge[0]
        theta = math.atan(slope)
        VisitedDict[OurMap[u]].append(theta)
        # print("Theta start to goal: \n", theta * 180/3.14)
        # need to calc theta dot still and also account for theta_start
...
def roadmap(ID):
    ...
    # still need to account for velocity_dot somehow
    Get_Thetas(OurMap)  # still need to account for theta_dot though
...
def collision_free_edges(edge_open_list, alpha_i, q_nearest):
    ...
    trim = 0
    for each in range(0,
                      len(edge_open_list)):  # for each edge node in open list starting from q_nearest --> branching out
        if in_obstacles(edge_open_list[each]) == True:  # check if the edge node is in obst
            clean_edge_open_list = edge_open_list[
                                   :each]  # cut off the rest of the edge open list (rest of the branch) after collision det
            trim = 1
...
for i in range(0, 200):
    ...
    if clean_edge_open_list != []:
        for clean in clean_edge_open_list: edges.append(
            clean)  # can now add the open edge list to the master edge list
        ...
        if once == 0:  # prob wont need this as a flag
            ...
            if not_straight == 0:
                ...
                edge_open_list = order_open_list(edge_open_list, alpha_i)
                for child in edge_open_list:
                    VisitedDict[child] = []
                    VisitedDict[child].append(alpha_i)
                alpha_i_new, clean_edge_open_list = collision_free_edges(edge_open_list, q_f, alpha_i)
                if clean_edge_open_list != []:
                    ...
                    VisitedDict[alpha_i_new].append(alpha_i)  # make nearest node in tree the parent of alpha_i
